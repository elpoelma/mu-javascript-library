...
const SC2 = require("sparql-client-2");
...
function newSparqlClient() {
	...
	if (httpContext.get("response")) {
		...
		if (allowedGroups)
			options.requestDefaults.headers["mu-auth-allowed-groups"] =
				allowedGroups;
	}
	...
	return newSparqlClient()
		...
		.then((response) => {
			...
			if (
				...
				const allowedGroups =
					response.headers["mu-auth-allowed-groups"];
				if (allowedGroups) {
					httpContext
						.get("response")
					...
					if (DEBUG_AUTH_HEADERS) {
						console.log(
							`Update mu-auth-allowed-groups to ${allowedGroups}`
						);
					}
				} else {
					httpContext
						.get("response")
						.removeHeader("mu-auth-allowed-groups");
					if (DEBUG_AUTH_HEADERS) {
				...
				if (usedGroups) {
					...
					if (DEBUG_AUTH_HEADERS) {
						console.log(
							`Update mu-auth-used-groups to ${usedGroups}`
						);
					}
				} else {
					httpContext
						.get("response")
						.removeHeader("mu-auth-used-groups");
					if (DEBUG_AUTH_HEADERS) {
...
function sparqlEscape(value, type) {
	switch (type) {
		...
		default:
			console.error(
				`WARN: Unknown escape type '${type}'. Escaping as string`
			);
			return sparqlEscapeString(value);
	}
}
